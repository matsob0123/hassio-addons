...
## Reads /data/options.json (Home Assistant add-on options) and
## writes the corresponding environment variables into /var/run/s6/container_environment/
## so they are visible to every s6 service.
##
...
# Write a single env var to the s6 environment directory.
# Usage: write_env VAR_NAME value
write_env() {
    local var="$1"
    local val="$2"
...
# Read a value from options.json; return default if key is null or missing.
# Usage: opt KEY [default]
opt() {
    local key="$1"
    local default="${2:-}"
    local val
...
write_env TZ "$TZ_VAL"
# Also apply at OS level for correct log timestamps
...
write_env AUTO_DISC_RIPPER_FORCE_UNIQUE_OUTPUT_DIR \
          "$(adr auto_disc_ripper_force_unique_output_dir)"
...
# Dark mode is passed via X11 resource; write it so the service can pick it up
